		...
		for (var i = 0; i < data.length; i++) {
			$("#tableThuongHieu").append(
				...
				<td><input type="button" class="btn btn-primary table__btn"
					value="Chi tiết"
					onclick="location.href='/websiteVali/admin/thuong-hieu/chi-tiet-thuong-hieu?id=${ data[i].id }'">

					<input type="button" class="btn btn-warning table__btn"
					value="Sửa"
					onclick="location.href='/websiteVali/admin/thuong-hieu/cap-nhat-thuong-hieu?id=${ data[i].id }'">

					<input type="button" class="btn btn-danger table__btn"
					value="Xóa"
					onclick="location.href='/websiteVali/admin/thuong-hieu/xoa-thuong-hieu?id=${ data[i].id }'">
	...
	$.ajax({
		...
		success: function (result) {
			if(result == 1){
				...
				window.location.href = 'http://localhost:8080/websiteVali/admin/thuong-hieu/danh-sach-thuong-hieu';
