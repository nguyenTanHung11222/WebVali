		...
		for (var i = 0; i < data.length; i++) {
			$("#tableLoaiSanPham").append(
				...
				<>
					...
					value="Chi tiết"
					onclick="location.href='/websiteVali/admin/loai-san-pham/chi-tiet-loai-san-pham?id=${data[i].id}'">

					<input type="button" class="btn btn-warning table__btn"
					value="Sửa"
					onclick="location.href='/websiteVali/admin/loai-san-pham/cap-nhat-loai-san-pham?id=${data[i].id}'">
	...
	$.ajax({
		...
		success: function (result) {
			if(result == 1){
				...
				window.location.href = 'http://localhost:8080/websiteVali/admin/loai-san-pham/danh-sach-loai-san-pham';
