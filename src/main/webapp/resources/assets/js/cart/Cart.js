...
add.each(function (index) {
    ...
    $(this).click(function () {
    	...
    	let apiFetch = "http://localhost:8080/websiteVali/api/gio-hang/kiem-tra-so-luong-ton?maSanPham=" + msp + "&maMau=" + mm;
...
xoaChiTietHoaDon = (maSanPham, maMau) =>{
	...
	$.ajax({
	    url: apiFetch = "http://localhost:8080/websiteVali/api/gio-hang/xoa-gio-hang?maSanPham=" + maSanPham + "&maMau=" + maMau,
	    type: 'DELETE',
	    success: function(result) {
	    	
	    	window.location.href = "http://localhost:8080/websiteVali/gio-hang";
...
capNhatSoLuongChiTietHoaDon = (maSanPham, maMau, soLuong) =>{
	...
	$.ajax({
		url: "http://localhost:8080/websiteVali/api/gio-hang/cap-nhat-so-luong?maSanPham=" + maSanPham + "&maMau=" + maMau + "&soLuong=" + soLuong,
		type: 'GET',
	});
}
